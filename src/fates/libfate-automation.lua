...
function BuildFateTable(fateObj)
    ...
    if IsCollectionsFate(fateTable.fateName) then
        fateTable.collectionsFateCtx = { haveMaxCredit = false }
    end
    fateTable.bossFateCtx = IsBossFate(fateTable.fateObject)
    fateTable.otherNpcFateCtx = IsOtherNpcFate(fateTable.fateName)
    fateTable.specialFateCtx = IsSpecialFate(fateTable.fateName)
...
function FateAutomation:potCheck()
    if not HasStatusId(49) and Potion ~= nil and Potion ~= "" then
    ...
    return {
        Exec = function()
            ...
            self.teleportManager = NewTeleportManager(
                60,
                function(message) self:logVerbose(message) end
            )
        end,
    ...
    return {
        Exec = function()
            ...
            if self.Config.ChocoboStance ~= nil
                ...
                and (
                    not shouldWaitForBonusBuff
                    or Inventory.GetItemCount(GYSAHLGREENS_ITEMID) > 0
                ) then
                self:updateState(AutomationState.SummonChocobo)
                return
            end
            if self.currentFate == nil or IsFateActive(self.currentFate.fateObject) then
...
function FateAutomation:updateDoFate(fate)
    self.currentFate = BuildFateTable(fate)
...
function FateAutomation:middleOfFateDismount()
    ...
    return {
        Exec = function()
            if not IsFateActive(self.currentFate) then
                self:updateState(AutomationState.Ready)
                return
            end
            if Svc.Targets.Target == nil then
...
function FateAutomation:fateCmp(fateA, fateB)
    local comparisons = {
        ...
        [FateCriteria.DistanceToTeleport] = function()
            return cmpNumber(
                -GetDistanceToPointWithAetheryteTravel(fateA.position),
                -GetDistanceToPointWithAetheryteTravel(fateB.position)
            )
        end,
    }
...
function FateAutomation:selectNextFate()
    ...
    for i = 0, fates.Count-1 do
        local tpFate = BuildFateTable(fates[i])
        if (tpFate.position.X == 0 and tpFate.position.Z == 0) then
            self:logVerbose("Found fate with zero coords ("..tpFate.fateName.."). Skipping..")
        elseif tpFate.isBlacklistedFate then
            self:logVerbose("found blacklisted fate ("..tpFate.fateName.."). Skipping..")
        elseif tpFate:isBossFate() and (
                (tpFate:isSpecialFate() and tpFate.fateObject.Progress < CompletionToJoinSpecialBossFates) and
                (not tpFate:isSpecialFate() and tpFate.fateObject.Progress < CompletionToJoinBossFate)
            ) then
            self:logVerbose("Boss fate is not at required completion percent ("..tpFate.fateName.."). Skipping..")
        elseif tpFate.duration == 0 then
            self:logVerbose("Found fate with duration zero ("..tpFate.fateName.."). Skipping..")
        else
        ...
        Exec = function()
            ...
            if GetDistanceToPoint(targetPos) > 5 then
                ...
                elseif not IPC.vnavmesh.PathFindInProgress() and not IPC.vnavmesh.IsRunning() then
                    if Player.CanFly and ~disableFly then
                        yield("/vnav flyflag")
                    else
                        yield("/vnav moveflag")
                    end
    ...
    return {
        Exec = function()
            if self.currentFate == nil then
                self:logVerbose("current fate unset, returning to ready")
                self:updateState(AutomationState.Ready)
            end
            if self.currentFate.npcName == nil then
                self:logVerbose("no npc set, returning to ready")
                self:updateState(AutomationState.Ready)
            end
    ...
    return {
        Exec = function()
            ...
            elseif IsFateActive(self.currentFate.fateObject)
                ...
                yield("/vnav stop")
                self:logVerbose("pushed out of fate -> pathing back")
    ...
    return {
        Exec = function()
            ...
            elseif os.clock() - waitStart > 30 then
                self:logVerbose("expected continuation fate timed out")
                self:updateState(AutomationState.Ready)
            else
                if self.Config.BossFatesJob ~= nil then
                    local currentJob = Player.Job.Id
...
function FateAutomation:changeInstance()
    ...
    return {
        Exec = function()
            self:logVerbose("feature disabled")
            self:updateState(AutomationState.Ready)
        end,
        Dispose = function() end
    }
    ...
    return {
        Exec = function()
            ...
            if Svc.ClientState.TerritoryType ~= 129 then
                ...
                yield("/vnav stop")
                self.teleportManager("Limsa Lominsa Lower Docks")
                return
            end
...
function FateAutomation:logVerbose(message)
    Dalamud.LogVerbose("[FATE] "..message)
end
...
function FateAutomation:updateState(newState)
    self:logVerbose("update state to "..newState)
    local jTable = {
